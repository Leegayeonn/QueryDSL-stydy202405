...
//        teamRepository.save(teamB);
    ...
    void testQueryDSL() {
        // given
        // QueryDSL 에서 사용할 Q클래스 받아온다. (클래스 내에 상수로 객체가 선언되어 있기 떄문에 직접 생성할 필요없음)
        QMember m = member;
    ...
    void search() {
        ...
        // when
        Member foundMember = factory.selectFrom(member)  // SELECT * FROM Member 전체 조회시
//                .where(member.userName.eq(searchName), member.age.eq(searchAge))
    ...
    void testFetchResult() {
        ...
        // fetchFirst 전체 조회되지만 리스트에서 첫번째만 출력
    ...
    void queryDslCustom() {
        ...
        assertEquals("teamB", members.get(0).getTeam().getName());
    }
...
//                .where(원하는 조건)
    ...
    void aggregation() {
        ...
        List<Tuple> result = factory.select(
                ...
                .from(member)
                .fetch();
        // Tuple 객체 : QueryDSL 에서 쿼리로 나온 결과 행들을 타입에 맞춰 담을 수 있게 제공되는 타입.
    ...
    @Test
    @DisplayName("suc query 사용하기 (나이가 가장 많은 회원을 조회)")
    void suvQueryTest() {
        // given
        // 같은 테이블에서 서브쿼리를 적용하려면 별도의 QClass 객체를 생성해야 합니다.
